...
class ReportListOfSpecializedtradeExpiry  extends ReportController{
    public function getIndex(){
        ...
        $query = "select SPNo,Status, CIDNo,Name, ApprovedDate,RegistrationExpiryDate, Dzongkhag, Country, Gewog,Village,Email, MobileNo from viewlistofspecializedtrade Where Status = 'Approved' AND RegistrationExpiryDate BETWEEN CURDATE() and DATE_ADD(CURDATE(), INTERVAL 30 DAY)   AND 1";
        ...
        if(Input::has('export')){
            $specializedtradeList = DB::select($query." order by RegistrationExpiryDate",$parameters);
            $export = Input::get('export');
            if($export == 'excel'){
                Excel::create('List of Specialized Trade nearing expiry', function($excel) use ($specializedtradeList, $country,$dzongkhag,$arNo,$limit,$fromDate,$toDate,$status) {
                    ...
                    $excel->sheet('Sheet 1', function($sheet) use ($specializedtradeList, $country,$dzongkhag,$arNo,$limit,$fromDate,$toDate,$status) {
                        ...
                        $sheet->loadView('reportexcel.listofspecializedtradeexpiry')
            ...
            $specializedtradeList = DB::select($query." order by RegistrationExpiryDate".$limitOffsetAppend,$parameters);
        }
        return View::make('report.listofspecializedtradeexpiry')
                        ->with('start',$start)
