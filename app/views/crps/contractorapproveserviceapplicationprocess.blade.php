...
<div id="sendback" class="modal fade" role="dialog" aria-labelledby="sendback" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<input type="hidden" name="RedirectUrl" value="contractor/approveserviceapplicationlist"/>
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
...
<div class="portlet light bordered">
	<div class="portlet-title">
		...
		<div class="actions">
			<a href="{{URL::to('contractor/approveserviceapplicationlist')}}" class="btn btn-default btn-sm">Back to List</a>
		</div>
	</div>
	<div class="portlet-body form">
		...
		<input type="hidden" name="RedirectRoute" value="approveserviceapplicationlist">
		<input type="hidden" name="ContractorReference" value="{{$generalInformation[0]->Id}}" />
		@if($hasFee)
		...
		<div class="row">
			<div class="col-md-12">
				...
				<table class="table table-bordered table-striped table-condensed flip-content">
					...
					<tbody>
						<?php $hasChangeOfOwner = false; $totalFeeApplicable=0; $categoryClassificationFeeTotal=0;$lateFeeAmount=0; $appliedServiceName = "";?>
						@foreach($appliedServices as $appliedService)
							...
							@if($appliedService->Id==CONST_SERVICETYPE_RENEWAL || $appliedService->Id==CONST_SERVICETYPE_LATEFEE || $appliedService->Id==CONST_SERVICETYPE_CHANGEOFCATEGORYCLASSIFICATION)
								<?php $appliedServiceName = $appliedService->ServiceName; ?>
							@endif
						<tr>
													...
													<td>
														@if((bool)$hasCategoryClassificationFee->VerifiedClassification!=NULL)
															@if($hasCategoryClassificationFee->VerifiedClassificationId==$hasCategoryClassificationFee->FinalApprovedClassificationId)
																<?php $categoryClassificationFeeTotal+=$hasCategoryClassificationFee->VerifiedRenewalFee; ?>
																{{number_format($hasCategoryClassificationFee->VerifiedRenewalFee,2)}}
															@else
																<?php $categoryClassificationFeeTotal+=$hasCategoryClassificationFee->VerifiedRegistrationFee; ?>
																{{number_format($hasCategoryClassificationFee->VerifiedRegistrationFee,2)}} 
															@endif
														@else 
															-
														@endif		
								...
								<table class="table table-hover table-condensed">
									...
									<tbody>
										...
										<tr>
											...
											<td colspan="3">
												<div class="form-group">
													<input type="text" disabled class="form-control number input-sm input-medium" value="@if($generalInformation[0]->WaiveOffLateFee == 1){{{$generalInformation[0]->NewLateFeeAmount}}}@endif" name="NewLateFeeAmount" placeholder="New Late Fees Amount">
												</div>
											</td>
										</tr>
									</tbody>
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										<input type="checkbox" name="ContractorHumanResourceModel[{{$randomKey}}][Approved]" class="checkboxes" checked="checked" value="1"/>
									</td>
								</tr>
								@empty
									<tr>
										<td colspan="10" class="text-center font-red">No new partners/owners</td>
									</tr>
								@endforelse
							</tbody>
						</table>	
						<a href="{{URL::to('contractor/editgeneralinfo/'.$generalInformation[0]->Id.'?redirectUrl=contractor/approveserviceapplicationprocess')}}" class="btn blue-madison">Edit General Information/Partner Details</a>								
					</div>
				</div>
				<div class="tab-pane" id="workclassification">
					...
					<table id="workclasssficationverification" class="table table-bordered table-striped table-condensed flip-content">
						...
						<tbody>
							...
							<tr>
								<td>
									@if($appliedServiceName != '')
										<input type="hidden" name="ContractorWorkClassificationModel[{{$randomKey}}][ServiceType]" value="{{$appliedServiceName}}"/>
									@else
										{{$appliedServiceName}}
									@endif
					...
					{{---<a href="{{URL::to('contractor/editworkclassification/'.$generalInformation[0]->Id.'?redirectUrl=contractor/approveserviceapplicationprocess')}}" class="btn blue-madison">Edit Work Classification</a>---}}
				</div>
				<div class="tab-pane" id="humanresource">
					<div class="table-responsive">
						...
						<table class="table table-bordered table-striped table-condensed flip-content">
							...
							<tbody>
								<?php $hasDeleteRequest = false; ?>
								@foreach($contractorHumanResourcesFinal as $contractorHumanResourceFinal)
								<tr>
									<td>
										{{Form::hidden('Id',$contractorHumanResourceFinal->Id,array('class'=>'rowreference'))}}
										{{$contractorHumanResourceFinal->Salutation.' '.$contractorHumanResourceFinal->Name}}
									</td>
									...
									<td>
										@if($contractorHumanResourceFinal->DeleteRequest == 1)
											<?php $hasDeleteRequest = true; ?>
											<i class="fa fa-check font-red"></i> &nbsp;
										@endif
										<a href="#" class="deletedbrow deleterequest">
											Delete
										</a>
									</td>
								</tr>
					...
					<a href="{{URL::to('contractor/edithumanresource/'.$generalInformation[0]->Id.'?redirectUrl=contractor/approveserviceapplicationprocess')}}" class="btn blue-madison">Edit Human Resource</a>
				</div>
				<div class="tab-pane" id="equipment">
					...
					<h5 class="font-blue-madison bold">Existing Equipment</h5>
						<table id="Equipment" class="table table-bordered table-striped table-condensed flip-content">
							{{Form::hidden('Model','ContractorEquipmentFinalModel',array('class'=>'delete-model'))}}
							<input type="hidden" id="application-id" value="{{$generalInformation[0]->Id}}"/>
							<thead>
								<tr>
									<th>
										Equipment Name
					...
					<a href="{{URL::to('contractor/editequipment/'.$generalInformation[0]->Id.'?redirectUrl=contractor/approveserviceapplicationprocess')}}" class="btn blue-madison">Edit Equipment</a>
				</div>
			</div>
		</div>
		<div class="portlet light bordered">
				...
				<div class="form-body">
					<div class="row">
						@if($hasDeleteRequest)
							<div class="col-md-12">
								<div class="form-group">
									<label class="control-label"><input type="checkbox" class="form-control" name="SendNotification"/> Send notification to replace HR after 30 days </label>
								</div>
							</div>
						@endif
						<div class="col-md-12">
							<h5 class="font-red bold">*Validity of CDB Certificate for contractors is {{$registrationValidityYears}} years.</h5>
						</div>
						<div class="col-md-4">
							<div class="form-group">
								<label class="control-label">Application Approved Date</label>
						...
						@if($hasRenewal)
							<div class="col-md-4">
								<div class="form-group">
									...
									<div class="input-icon right">
										<i class="fa fa-calendar"></i>
										<input type="text" class="form-control required" placeholder="" value="{{convertDateToClientFormat($generalInformationFinal[0]->RegistrationExpiryDate)}}" readonly="readonly" />
									</div>
								</div>
							</div>
							<div class="col-md-4">
								<div class="form-group">
									...
									<div class="input-icon right">
										<i class="fa fa-calendar"></i>
										<input type="text" name="RegistrationExpiryDate" class="form-control datepicker required" placeholder="" value="{{registrationExpiryDateCalculatorRenewal(date('Y-m-d G:i:s'),$registrationValidityYears)}}" readonly="readonly" />
									</div>
								</div>
							</div>
						@endif
