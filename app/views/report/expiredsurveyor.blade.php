...
<div class="portlet light bordered">
	<div class="portlet-title">
		<div class="caption">
			...
			<i class="fa fa-cogs"></i>List of Expired Surveyors &nbsp;&nbsp;@if(!Input::has('export')) <a href="{{route('surveyorrpt.expiredsurveyor',$parameters)}}" class="btn btn-sm green"><i class="fa fa-file-excel-o"></i> Export to Excel</a>    <?php $parameters['export'] = 'print'; ?><a href="{{route('surveyorrpt.expiredsurveyor',$parameters)}}" class="btn btn-sm purple" target="_blank"><i class="fa fa-print"></i> Print</a>@endif
		</div>
	</div>
	<div class="portlet-body flip-scroll">
					...
					<div class="form-group">
						...
						<select class="form-control" name="SectorType">
                            ...
                            @foreach($sectors as $sector)
                                <option value="{{$sector->Name}}" @if($sector->Name == Input::get('SurveyType'))selected @endif>{{$sector->Name}}</option>
                            @endforeach
						</select>
					</div>
				</div>
            ...
            @forelse($surveyList as $survey)
				<tr>
              	...
              	<td >
							...
							@if($survey->ExpiryDate<=date('Y-m-d G:i:s'))
							<p class="font-red bold warning">Expired</p>
							@else
							{{convertDateToClientFormat($survey->ExpiryDate)}}
							@endif
        ...
        <?php pagination($noOfPages,Input::all(),Input::get('page'),"surveyorrpt.expiredsurveyor"); ?>
	</div>
</div>
@stop