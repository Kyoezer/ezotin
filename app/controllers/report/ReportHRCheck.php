    ...
    public function getIndex(){
        ...
        if((bool)$cidNo){
            ...
            $hrWorksCinet = DB::table('cinetbidhumanresource as T1')
                            ...
                            ->where('T1.CIDNo',$cidNo)
                            ->groupBy('T2.Id')
                            ->get(array(DB::raw('distinct(T2.Id)'),DB::raw('group_concat(B.CDBNo SEPARATOR ",") as CDBNo'),DB::raw('coalesce(T2.ReferenceNo) as WorkId'),'T4.Name as ProcuringAgency'));
            ...
            if(count($hrWorksCinet) > 0){
                $workId = DB::table('cinetbidhumanresource as T1')
                            ...
                            ->where('T1.CIDNo',$cidNo)
                            ->groupBy('T1.Id')
                            ->get(array(DB::raw('distinct(T1.Id)'),DB::raw('group_concat(coalesce(T2.ReferenceNo) SEPARATOR ",") as WorkId')));
