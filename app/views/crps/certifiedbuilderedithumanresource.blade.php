...
{{ Form::open(array('url' => 'certifiedbuilder/mcertifiedbuilderhumanresource','role'=>'form','files'=>true))}}
...
<div class="portlet light bordered">
    ...
    <div class="table-responsive">
		...
		<table class="table table-bordered table-striped table-condensed flip-content">
			...
			<tbody>
				@forelse($certifiedbuilderHumanResourcesFinal as $certifiedbuilderHumanResourceFinal)
				<tr>
					<td>
						<input type="hidden" class="rowreference" value="{{$certifiedbuilderHumanResourceFinal->Id}}"/>
						{{$certifiedbuilderHumanResourceFinal->Name}}
					</td>
					<td>{{$certifiedbuilderHumanResourceFinal->CIDNo}}</td>
					<td>{{$certifiedbuilderHumanResourceFinal->Sex}}</td>
					<td>{{$certifiedbuilderHumanResourceFinal->Country}}</td>
					<td>{{$certifiedbuilderHumanResourceFinal->Qualification}}</td>
				
					<td>{{$certifiedbuilderHumanResourceFinal->Designation}}</td>
					<td>{{$certifiedbuilderHumanResourceFinal->JoiningDate}}</td>
					<td>{{$certifiedbuilderHumanResourceFinal->Trade}}</td>
					<td>
						@foreach($humanResourcesAttachmentsFinal as $humanResourcesAttachmentFinal)
						@if($certifiedbuilderHumanResourceFinal->Id==$humanResourcesAttachmentFinal->CrpCertifiedbuilderHumanResourceFinalId)
					...
					<td>
						<center><input type="checkbox" class="senddeleterequest" data-type="1" value="{{$certifiedbuilderHumanResourceFinal->Id}}"></center>
					</td>
					{{--@if((int)$serviceByCertifiedbuilder==0)--}}
					<td>
						@if((bool)$isEdit!=null)
							@if(!empty($isEditByCDBUser))
							<a href="{{URL::to($humanResourceEditRoute.'/'.$isEdit.'?humanresourceid='.$certifiedbuilderHumanResourceFinal->Id.'&redirectUrl='.$redirectUrl.'&usercdb=edit')}}" class="editaction">Edit</a>
							@else
								<a href="{{URL::to($humanResourceEditRoute.'/'.$isEdit.'?humanresourceid='.$certifiedbuilderHumanResourceFinal->Id.'&redirectUrl='.$redirectUrl)}}" class="editaction">Edit</a>
							@endif
...
{{--					@if((int)$serviceByCertifiedbuilder==0)--}}
			...
			<tbody>
				...
				<tr>
					<td>
						<input type="hidden" class="rowreference" value="{{$certifiedbuilderHumanResource->Id}}">
						{{$certifiedbuilderHumanResource->Name}}
					</td>
					...
					<td>{{$certifiedbuilderHumanResource->JoiningDate}}</td>
					<td>{{$certifiedbuilderHumanResource->Trade}}</td>
					<td>
						@foreach($humanResourcesAttachments as $humanResourcesAttachment)
						@if($certifiedbuilderHumanResource->Id==$humanResourcesAttachment->CrpCertifiedbuilderHumanResourceId)
					...
					<td>
						<input type="hidden" class="recordreference" value="{{$certifiedbuilderHumanResource->Id}}">
						<a href="{{URL::to($humanResourceEditRoute.'/'.$isEdit.'?humanresourceid='.$certifiedbuilderHumanResource->Id.'&redirectUrl='.$redirectUrl.'&usercdb=edit&initial=true')}}" class="editaction">Edit</a> |
	...
	<div class="form-actions">
		<div class="btn-set">
			...
			<a href="{{URL::to($nextUrl.$certifiedbuilderId)}}" class="btn blue button-next">
