...
class ReportListOfArchitects extends ReportController{
    public function getIndex(){
        ...
        $query = "select ARNo,Sector,Status, CIDNo,ArchitectName, ApprovedDate,ExpiryDate, Dzongkhag, Country, Gewog,Village,Email, MobileNo from viewlistofarchitects Where 1";
        ...
        if(Input::has('export')){
            ...
            if($export == 'excel'){
                Excel::create('List of Architects', function($excel) use ($architectsList, $country,$dzongkhag,$arNo,$sector,$limit,$fromDate,$toDate,$status) {
                    ...
                    $excel->sheet('Sheet 1', function($sheet) use ($architectsList, $country,$dzongkhag,$arNo,$sector,$limit,$fromDate,$toDate,$status) {
                        ...
                        $sheet->loadView('reportexcel.listofarchitects')
        ...
        return View::make('report.listofarchitects')
                        ->with('start',$start)
