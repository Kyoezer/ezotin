<?php
class NewetoolWorkIdEtool extends EtoolController{
	public function index(){
        ...
        $query = "select distinct T1.Id,T1.Method,T1.TenderStatus,T1.EGPTenderId, T1.TenderOpeningDateAndTime, T2.Code as Category, T3.Code as Classification,
         T1.NameOfWork, T1.ContractPeriod, case when T1.migratedworkid is null then
          ...
          and T1.TenderSource = 1 and coalesce(T1.DeleteStatus,'N') <> 'Y' and T1.IsSPRRTender='Y'";
        ...
        where T1.CmnProcuringAgencyId = ? and T1.TenderSource = 1 and coalesce(T1.DeleteStatus,'N') <> 'Y'  and T1.IsSPRRTender='Y'";
...
//        echo "<pre>"; dd($distinctYears);
        ...
        return View::make('new_etool.workid')
