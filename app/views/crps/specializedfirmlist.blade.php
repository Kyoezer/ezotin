			...
			<div class="modal-body">
				...
				<div class="col-md-4">
					<div class="form-group">
						<input type="hidden" name="Model" value="SpecializedfirmFinalModel"/>
						<input type="hidden" name="RedirectUrl" value="specializedfirm/editlist"/>
...
<th>Status</th>
			...
			<td>
						@if((int)$specializedfirmList->StatusReference == 12003)
							@if($specializedfirmList->RegistrationExpiryDate<=date('Y-m-d G:i:s'))
									<p class="font-red bold warning">Expired </p>
							@else
								Valid/Approved
							@endif
						@else
							<p class="font-red bold warning">{{$specializedfirmList->Status}}</p>
						@endif
					</td>
					<!-- <td>
					...
					<td>
						<a href="{{URL::to($link.$specializedfirmList->Id)}}" class="btn default btn-xs green-seagreen @if($linkText=="Edit"){{"editaction"}}@endif" @if($linkText=="View/Print"){{"target='_blank'"}}@endif><i class="fa fa-edit"></i> {{$linkText}}</a>
					</td>
					<td>
						<a href="#changeexpirydatemodal" data-expirydate="{{convertDateToClientFormat($specializedfirmList->RegistrationExpiryDate)}}" data-id="{{$specializedfirmList->Id}}" data-toggle="modal" class="change-expirydate btn default btn-xs green"><i class="fa fa-edit"></i> Change</a>
					</td>
				</tr>
				@empty
				<tr>
