...
<div class="portlet light bordered">
	...
	<div class="portlet-body">
		...
		<div class="row">
			<div class="col-md-6">
				<h5 class="font-blue-madison bold">Specializedfirm who has been awarded the work</h5>
				<table class="table table-bordered table-striped table-condensed flip-content">
					<thead>
						<tr>
							<th>Name of Specializedfirm</th>
							<th>SP No</th>
					...
					<tbody>
						...
						@foreach($workAwardedSpecializedfirm as $workAwardedSpecializedfirm)
						<?php $bidAmountTotal+=$workAwardedSpecializedfirm->BidAmount;$evaluatedAmountTotal+=$workAwardedSpecializedfirm->EvaluatedAmount; ?>
						<tr>
							<td>{{$workAwardedSpecializedfirm->NameOfFirm}}</td>
							<td>{{$workAwardedSpecializedfirm->SPNo}}</td>
							<td class="text-right">{{number_format($workAwardedSpecializedfirm->BidAmount,2)}}</td>
							<td class="text-right">{{number_format($workAwardedSpecializedfirm->EvaluatedAmount,2)}}</td>
						</tr>
						@endforeach
				...
				<div class="row">
					<div class="col-md-3">
						<div class="form-group">
							...
							<input type="text" name="ContractPriceInitial" class="form-control text-right input-sm workstatuscompletedcontrol required number" value="{{$detailsOfCompletedWork->ContractPriceInitial}}" />
						</div>
					</div>
					<div class="col-md-3">
						<div class="form-group">
							<label class="control-label">Contract Price (Final) Nu.</label>
							<input type="text" name="ContractPriceFinal" class="form-control text-right input-sm workstatuscompletedcontrol required number" value="{{$detailsOfCompletedWork->ContractPriceFinal}}" />
						</div>
					</div>
					<div class="col-md-3">
						<div class="form-group">
		...
		<div class="form-actions">
			<div class="row">
				<div class="col-md-12">
					...
					<a href="{{URL::to('specializedfirm/worklist')}}" class="btn red">Cancel</a>
				</div>
			</div>
		</div>
	</div>
