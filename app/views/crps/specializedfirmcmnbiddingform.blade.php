			...
			<div class="portlet-body form">
				{{ Form::open(array('url' => $model,'role'=>'form','class'=>'form-horizontal'))}}
					<div class="form-body">
						...
						<div class="form-group">
							...
							<div class="col-md-4">
								<select name="CmnSpecializedfirmCategoryId" class="form-control required input-sm">
									<option value="">---SELECT ONE---</option>
									@foreach($specializedfirmProjectCategories as $Category)
									<option value="{{$Category->Id}}" @if($Category->Id==$contractWorkDetail->CmnSpecializedfirmCategoryId || Input::old('CmnSpecializedfirmCategoryId')==$Category->Id)selected="selected"@endif>{{$Category->Code.' ('.$Category->Name.')'}}</option>
									@endforeach
								</select>
							</div>
						</div>
						...
						@endforeach
						<h5 class="bold font-blue-madison">Name of the @if((int)$type==2){{"Specializedfirm"}}@else{{"Consultants"}}@endif who Submitted Bids</h5>
							...
							<table id="ContractorAdd" class="table table-bordered table-striped table-condensed">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<input type="text" class="form-control input-sm cdbno" value="{{$contractBidder->SPNo}}">
										</td>
										<td>
											@if((int)$type==2)
											<input type="hidden" name="CrpBiddingFormDetailModel[{{$randomKey}}][CrpSpecializedTradeFinalId]" value="{{$contractBidder->CrpSpecializedTradeFinalId}}" class="resetKeyForNew specializedtrade-id" />
											<input type="text" class="specializedtrade-name form-control input-sm" value="{{$contractBidder->NameOfFirm}}" readonly="readonly"/>
											@endif
