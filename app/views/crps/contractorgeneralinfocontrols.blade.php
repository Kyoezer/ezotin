...
@foreach($contractorGeneralInfo as $generalInfo)
<div class="form-body">
	<div class="row">
		<div class="col-md-6">
	...
	<div class="row">
		...
		{{--<div class="clearfix"></div>--}}
		<div class="companyownershiptypeattachmentcontrol @if(empty($generalInfo->Id) || $generalInfo->CmnOwnershipTypeId=='1e243ef0-c652-11e4-b574-080027dcfac6' || $serviceByContractor==1){{'hide'}}@endif">
				...
				<table id="certificateofincorporation" class="table table-bordered table-striped table-condensed">
					...
					<tbody>
						<tr>
							...
							<td>
								<input type="text" name="DocumentName[]" value="Certificate of Incorporation" class="dontdisable form-control input-sm @if(empty($generalInfo->Id) && $serviceByContractor==0){{'required'}}@endif">
							</td>
							<td>
								<input type="file" name="attachments[]" class="dontdisable input-sm @if(empty($generalInfo->Id) && $serviceByContractor==0){{'required'}}@endif" multiple="multiple" />
							</td>
						</tr>
	...
	{{--<div class="row">--}}
		{{--<div class="col-md-6 table-responsive">--}}
			{{--<h5 class="font-blue-madison bold">Attach Refresher Course Certificate</h5>--}}
			{{--<table id="certificateofincorporation" class="table table-bordered table-striped table-condensed">--}}
				{{--<thead>--}}
				{{--<tr>--}}
					{{--<th>Document Name</th>--}}
		...
		<div class="form-body">
			...
			<div class="table-responsive">
				<table id="ownerpartnerdetails" class="table table-bordered table-striped table-condensed">
					...
					<tbody>
						<?php $count=1; ?>
						@forelse($contractorPartnerDetails as $contractorPartnerDetail)
						<?php $randomKey=randomString();?>
						<tr>
							<td>
								<input type="hidden" class="rowreference" value="{{$contractorPartnerDetail->Id}}">
								<button type="button" class="@if(Input::has('final')){{"deletedbrow"}}@else{{"deletetablerow"}}@endif"><i class="fa fa-times"></i></button>
							</td>
							<td>
								<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="text" name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][CIDNo]" value="{{$contractorPartnerDetail->CIDNo}}" data-id="owner-partner" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif unique changeofowner {{--checkhrtr--}} cidforwebservicetr">
							</td>
							<td>
								<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="hidden" name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][Id]" value="{{$contractorPartnerDetail->Id}}" class="resetKeyForNew changeofowner"/>
								<select @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][CmnSalutationId]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner">
									<option value="">---SELECT---</option>
									@foreach($salutations as $salutation)
									<option value="{{$salutation->Id}}" @if($salutation->Id==$contractorPartnerDetail->CmnSalutationId)selected="selected"@endif>{{$salutation->Name}}</option>
									@endforeach
								</select>
							</td>
							<td>
								<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="text" name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][Name]" value="{{$contractorPartnerDetail->Name}}" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner namefromwebservice">
							</td>
							<td>
								<select @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][Sex]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner sexfromwebservice">
									<option value="">---SELECT ONE---</option>
									<option value="M"  @if($contractorPartnerDetail->Sex=="M")selected="selected"@endif>Male</option>
									<option value="F"  @if($contractorPartnerDetail->Sex=="F")selected="selected"@endif>Female</option>
								</select>
							</td>
							<td>
								<select @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][CmnCountryId]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner notclearfornew countryfromwebservice">
									<option value="">---SELECT ONE---</option>
									@foreach($countries as $country)
										<option value="{{$country->Id}}" @if($contractorPartnerDetail->CmnCountryId==$country->Id || $country->Nationality=="Bhutanese")selected="selected"@endif>{{$country->Nationality}}</option>
									@endforeach
								</select>
							</td>
							<td>
								<select id="registration-hrdesignation" @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][CmnDesignationId]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner">
									<option value="">---SELECT ONE---</option>
									@foreach($designations as $designation)
										<option value="{{$designation->Id}}" data-reference="{{$designation->ReferenceNo}}" @if($contractorPartnerDetail->CmnDesignationId==$designation->Id)selected="selected"<?php if($designation->ReferenceNo == 101): $ownerEngineer = true; ?><?php endif; ?>@endif>{{$designation->Name}}</option>
									@endforeach
								</select>
							</td>
							<td>
								<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="checkbox" name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][ShowInCertificate]" class="checkboxes resetKeyForNew notclearfornew addrowcheckboxsinglecheck changeofowner" value="1" @if((int)$contractorPartnerDetail->ShowInCertificate==1)checked="checked"@endif/>
								<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="hidden" name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][IsPartnerOrOwner]" class="resetKeyForNew notclearfornew changeofowner" value="1"/>
							</td>
						</tr>
						<?php $count++; ?>
						@empty
							...
							<tr>
								...
								<td>
									<input type="text" @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][CIDNo]" value="" data-id="owner-partner" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif unique changeofowner {{--checkhrtr--}} cidforwebservicetr">
								</td>
								<td>
									<input type="hidden" @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][Id]" value="" class="resetKeyForNew changeofowner"/>
									<select @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][CmnSalutationId]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner">
										<option value="">---SELECT---</option>
										@foreach($salutations as $salutation)
								...
								<td>
									<input type="text" @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][Name]" value="" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner namefromwebservice">
								</td>
								<td>
									<select @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][Sex]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner sexfromwebservice">
										<option value="">---SELECT ONE---</option>
								...
								<td>
									<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="checkbox" name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][ShowInCertificate]" class="checkboxes resetKeyForNew notclearfornew addrowcheckboxsinglecheck changeofowner" value="1" checked="checked"/>
									<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="hidden" name="{{Input::has('final')?"ContractorHumanResourceFinalModel":"ContractorHumanResourceModel"}}[{{$randomKey}}][IsPartnerOrOwner]" class="resetKeyForNew notclearfornew changeofowner" value="1"/>
								</td>
							</tr>
						@endforelse
			...
			@if(!(bool)$isRejectedApp)
			<div class="row @if($ownerEngineer){{"hide"}}@endif" id="certificatewrapper">
				<div class="col-md-6 table-responsive">
			...
			@if(isset($editByCDB))
			@if(!(bool)$editByCDB)
			@if(Input::get('redirectUrl')!="contractor/verifyserviceapplicationprocess")
			<div class="row @if(!$ownerEngineer){{"hide"}}@endif" id="propcumengineerwrapper">
				<div class="col-md-6 table-responsive">
					...
					<div class="row">
						<div class="col-md-4">
							<div class="form-group">
								...
								<textarea name="LocationChangeReason" id="LocationChangeReason" class="form-control col-md-3" placeholder="Required only if you have applied for Change of Location">{{$generalInfo->LocationChangeReason}}</textarea>
							</div>
							<br><br><br>
						</div>
