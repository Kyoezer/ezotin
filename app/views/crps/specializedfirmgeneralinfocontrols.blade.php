...
@foreach($specializedtradeGeneralInfo as $generalInfo)
<div class="form-body">
	<div class="row">
		<div class="col-md-6">
	...
	<div class="row">
		...
		<div class="col-md-3">	
			<div class="form-group">
				<label class="control-label">Proposed Firm/Company Name <span class="font-red">* </span> <a role="button" tabindex="0" data-toggle="popover" data-container="body" data-trigger="focus" class="popoverdefaultopen" data-placement="top" title="<span class='font-green-seagreen'>Proposed Name</span>" data-content="<p class='text-justify'>If the Name of the Firm that you have proposed for specializedfirm registration is already registered with CDB you cannot proceed with registration. Try using a different Name in such cases.</p>"><i class="fa fa-question-circle"></i></a></label>
		...
		<div class="form-body">
			...
			<div class="table-responsive">
				<table id="ownerpartnerdetails" class="table table-bordered table-striped table-condensed">
					{{Form::hidden('Model','SpecializedfirmHumanResourceFinalModel',array('class'=>'delete-model'))}}
					<thead>
						<tr>
							<th>
							</th>
					...
					<tbody>
						<?php $count=1; ?>
						@forelse($specializedtradePartnerDetails as $specializedtradePartnerDetail)
						<?php $randomKey=randomString();?>
						<tr>
							<td>
								<input type="hidden" class="rowreference" value="{{$specializedtradePartnerDetail->Id}}">
								<button type="button" class="@if(Input::has('final')){{"deletedbrow"}}@else{{"deletetablerow"}}@endif"><i class="fa fa-times"></i></button>
							</td>
							<td>
								<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="text" name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][CIDNo]" value="{{$specializedtradePartnerDetail->CIDNo}}" data-id="owner-partner" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif unique changeofowner {{--checkhrtr--}} cidforwebservicetr">
							</td>
							<td>
								<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="hidden" name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][Id]" value="{{$specializedtradePartnerDetail->Id}}" class="resetKeyForNew changeofowner"/>
								<select @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][CmnSalutationId]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner">
									<option value="">---SELECT---</option>
									@foreach($salutations as $salutation)
									<option value="{{$salutation->Id}}" @if($salutation->Id==$specializedtradePartnerDetail->CmnSalutationId)selected="selected"@endif>{{$salutation->Name}}</option>
									@endforeach
								</select>
							</td>
							<td>
								<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="text" name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][Name]" value="{{$specializedtradePartnerDetail->Name}}" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner namefromwebservice">
							</td>
							<td>
								<select @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][Sex]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner sexfromwebservice">
									<option value="">---SELECT ONE---</option>
									<option value="M"  @if($specializedtradePartnerDetail->Sex=="M")selected="selected"@endif>Male</option>
									<option value="F"  @if($specializedtradePartnerDetail->Sex=="F")selected="selected"@endif>Female</option>
								</select>
							</td>
							<td>
								<select @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][CmnCountryId]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner notclearfornew countryfromwebservice">
									<option value="">---SELECT ONE---</option>
									@foreach($countries as $country)
										<option value="{{$country->Id}}" @if($specializedtradePartnerDetail->CmnCountryId==$country->Id || $country->Nationality=="Bhutanese")selected="selected"@endif>{{$country->Nationality}}</option>
									@endforeach
								</select>
							</td>
							<td>
								<select id="registration-hrdesignation" @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][CmnDesignationId]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner">
									<option value="">---SELECT ONE---</option>
									@foreach($designations as $designation)
										<option value="{{$designation->Id}}" data-reference="{{$designation->ReferenceNo}}" @if($specializedtradePartnerDetail->CmnDesignationId==$designation->Id)selected="selected"<?php if($designation->ReferenceNo == 101): $ownerEngineer = true; ?><?php endif; ?>@endif>{{$designation->Name}}</option>
									@endforeach
								</select>
							</td>
							<td>
								<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="checkbox" name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][ShowInCertificate]" class="checkboxes resetKeyForNew notclearfornew addrowcheckboxsinglecheck changeofowner" value="1" @if((int)$specializedtradePartnerDetail->ShowInCertificate==1)checked="checked"@endif/>
								<input @if(Input::has('oldapplicationid'))disabled="disabled"@endif type="hidden" name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][IsPartnerOrOwner]" class="resetKeyForNew notclearfornew changeofowner" value="1"/>
							</td>
						</tr>
						<?php $count++; ?>
						@empty
							...
							<tr>
								...
								<td>
									<input type="text" @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][CIDNo]" value="" data-id="owner-partner" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif unique changeofowner {{--checkhrtr--}} cidforwebservicetr">
								</td>
								<td>
									<input type="hidden" @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][Id]" value="" class="resetKeyForNew changeofowner"/>
									<select @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][CmnSalutationId]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner">
										<option value="">---SELECT---</option>
										@foreach($salutations as $salutation)
								...
								<td>
									<input type="text" @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][Name]" value="" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner namefromwebservice">
								</td>
								<td>
									<select @if(Input::has('oldapplicationid'))disabled="disabled"@endif name="{{Input::has('final')?"SpecializedfirmHumanResourceFinalModel":"SpecializedfirmHumanResourceModel"}}[{{$randomKey}}][Sex]" class="form-control input-sm resetKeyForNew @if(!Input::has('oldapplicationid')){{"required"}}@endif changeofowner sexfromwebservice">
										<option value="">---SELECT ONE---</option>
			...
			<div class="form-actions">
				<div class="btn-set">
					...
					<button type="submit" class="btn blue">Update <i class="fa fa-arrow-circle-o-right"></i></button>
						...
						@else 
						<a href="{{URL::to('specializedfirm/confirmregistration')}}" class="btn red">Cancel <i class="fa fa-times"></i></a>
						@endif
					@endif
				</div>
			</div>
