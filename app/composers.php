...
View::composer('horizontalmenumaster', function($view){
	...
	if(in_array(CONST_ROLE_ADMINISTRATOR,$userRoles,true)){
		...
		->whereRaw('coalesce(T2.IsInactive,0)<>1 and T2.Id in (select SysMenuId from sysrolemenumap as A 
		where A.SysRoleId=? and A.PageView=1)',

		// ->whereRaw('coalesce(T2.IsInactive,0)<>1 and T2.Id in (select SysMenuId from sysrolemenumap as A 
		// join sysuserrolemap as B on A.SysRoleId=B.SysRoleId where B.SysRoleId=? and A.PageView=1)',
		array($curUserRole))->orderBy('T1.DisplayOrder')->orderBy('T2.DisplayOrder')->get(array('T2.MenuTitle','T2.MenuRoute'));
	}else{
		
		if(count($userRoles) == 2){
			if(in_array(CONST_ROLE_PROCURINGAGENCYCINET,$userRoles,true)
			&& (in_array(CONST_ROLE_PROCURINGAGENCYETOOL,$userRoles,true) 
			|| in_array(CONST_ROLE_PROCURINGAGENCYCBUILDER,$userRoles,true))){
			...
			else
			{
				...
				->join('sysmenu as T2','T1.Id','=','T2.SysMenuGroupId')
				->where('T1.MenuType',2)->where('T2.MenuDisplayType',1)
				->whereRaw('coalesce(T2.IsInactive,0)<>1 and T2.Id in (select SysMenuId from sysrolemenumap as A join sysuserrolemap as B on A.SysRoleId=B.SysRoleId where B.SysRoleId=? and A.PageView=1)',
				 array($curUserRole))->orderBy('T1.DisplayOrder')->orderBy('T2.DisplayOrder')
				 ->get(array('T2.MenuTitle','T2.MenuRoute'));
			}
		}
		else
		{
		...
		if(!$dualModuleCheck){
			...
			->whereRaw('coalesce(T2.IsInactive,0)<>1 and T2.Id in (select SysMenuId from sysrolemenumap as A
			 join sysuserrolemap as B on A.SysRoleId=B.SysRoleId where B.SysUserId=? and A.PageView=1)',
			 array($loggedInUser))->orderBy('T1.DisplayOrder')->orderBy('T2.DisplayOrder')->get(array('T2.MenuTitle','T2.MenuRoute'));
		}
		$emailId = DB::table('sysuser')->where('Id',$loggedInUser)->pluck('Email');
	}

//	echo '<PRE>'; dd($horizontalMenus);
    $module = Request::segment(1);
	//return 'hello';
