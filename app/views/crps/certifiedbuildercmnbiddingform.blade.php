				...
				{{ Form::open(array('url' => $model,'role'=>'form','class'=>'form-horizontal'))}}
					<div class="form-body">
						...
						<div class="form-group">
							...
							<div class="col-md-7">
								<input type="text" name="ReferenceNo" class="form-control input-sm" placeholder="ReferenceNo" value="@if(!empty($contractWorkDetail->ReferenceNo)){{$contractWorkDetail->ReferenceNo}}@else{{Input::old('ReferenceNo')}}@endif"/>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-md-3">Name Of Client</label>
							<div class="col-md-7">
								<input type="text" name="NameOfClient" class="form-control input-sm" placeholder="Name of Client" value="@if(!empty($contractWorkDetail->NameOfClient)){{$contractWorkDetail->NameOfClient}}@else{{Input::old('NameOfClient')}}@endif"/>
							</div>
						</div>
						...
						<!-- <div class="form-group">
							<label class="control-label col-md-3">Approved Client Estimate (Nu.)</label>
							<div class="col-md-4">
								<input type="text" name="ApprovedAgencyEstimate" class="form-control text-right required input-sm" placeholder="Approved Client Estimate" value="@if(!empty($contractWorkDetail->ApprovedAgencyEstimate)){{$contractWorkDetail->ApprovedAgencyEstimate}}@else{{Input::old('ApprovedAgencyEstimate')}}@endif" />
							</div>
						</div> -->
						...
						@endforeach
						<h5 class="bold font-blue-madison">Name of the @if((int)$type==2){{"Certified Builder"}}@endif</h5>
						<div class="table-responsive">
							<table id="CertifiedBuilderAdd" class="table table-bordered table-striped table-condensed">
								<thead>
									<tr>
										<th>
										</th>
								...
								<tbody>
									...
									<tr>
										...
										<td>
											@if((int)$type==2)
											<input type="hidden" name="CrpBiddingFormDetailModel[{{$randomKey}}][CrpCertifiedBuilderFinalId]" value="{{$contractBidder->CrpCertifiedBuilderFinalId}}" class="resetKeyForNew certifiedbuilder-id" />
											<input type="text" class="certifiedbuilder-name form-control input-sm" value="{{$contractBidder->NameOfFirm}}" readonly="readonly"/>
											@endif
						...
						<div class="table-responsive">
							...
							<table class="table table-striped table-condensed table-bordered" id="cinet-hr-table">
								...
								<tr>
									...
									<td>
										<input type="text" name="cbHR[{{$randomKey}}][CIDNo]" @if(!(bool)$bidHR->CIDNo)disabled="disabled"@endif value="{{$bidHR->CIDNo}}" class="form-control resetKeyForNew input-sm cidforwebservicetr cinet-hr checkHumanResource"/>
									</td>
									<td>
										<input type="text" name="cbHR[{{$randomKey}}][Name]" @if(!(bool)$bidHR->CIDNo)disabled="disabled"@endif value="{{$bidHR->Name}}" class="form-control resetKeyForNew input-sm namefromwebservice"/>
									</td>
									<td>
										<select name="cbHR[{{$randomKey}}][CmnDesignationId]" @if(!(bool)$bidHR->CIDNo)disabled="disabled"@endif class="form-control resetKeyForNew input-sm">
									...
									<td>
										<select name="cbHR[{{$randomKey}}][CmnQualificationId]" @if(!(bool)$bidHR->CIDNo)disabled="disabled"@endif class="form-control resetKeyForNew input-sm">
											<option value="">SELECT</option>
						...
						<table class="table table-striped table-condensed table-bordered" id="cinet-eq-table">
							...
							@foreach($bidEquipments as $bidEquipment)
								<tr>
									...
									<td>
										<select name="cbEquipment[{{$randomKey}}][CmnEquipmentId]" @if(!(bool)$bidEquipment->CmnEquipmentId)disabled="disabled"@endif class="form-control resetKeyForNew input-sm equipmentforwebservicetr">
											<option value="">SELECT</option>
									...
									<td>
										{{Form::select("cbEquipment[$randomKey][OwnerOrHired]",array(''=>'SELECT','1'=>'Owned','2'=>'Hired'),$bidEquipment->OwnedOrHired,array('class'=>'form-control resetKeyForNew input-sm',(!(bool)$bidEquipment->CmnEquipmentId)?"disabled":'data-x'=>"disabled"))}}
									</td>
									<td>
										<input type="text" value="{{$bidEquipment->RegistrationNo}}" @if(!(bool)$bidEquipment->CmnEquipmentId)disabled="disabled"@endif name="cbEquipment[{{$randomKey}}][RegistrationNo]" class="form-control resetKeyForNew input-sm notrequired registration-no checkEquipment" />
									</td>
								</tr>
							@endforeach
								<tr class="notremovefornew">
