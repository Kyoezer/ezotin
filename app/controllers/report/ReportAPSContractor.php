...
class ReportAPSContractor extends ReportController{
    public function getIndex(){
        ...
        $query = "select T1.CDBNo,T1.Contractor, T1.WorkId, year(T1.WorkCompletionDate) as Year, T1.ProcuringAgency as Agency,T1.NameOfWork,T1.ProjectCategory as Category,T1.AwardedAmount,T1.ContractPriceFinal as FinalAmount,T1.Dzongkhag, T1.WorkStatus as Status, coalesce(T1.OntimeCompletionScore,0) as OntimeCompletionScore, coalesce(T1.QualityOfExecutionScore) as QualityOfExecutionScore from viewcontractorstrackrecords T1 where T1.ReferenceNo = 3003";
        $parameters = array();
        if((bool)$fromDate){
        ...
        if(Request::path() == "etoolrpt/apscontractor"){
            $type = 1;
            if((bool)$to){
        ...
        if(Input::has('export')) {
            ...
            if ($export == 'excel') {
                Excel::create('APS Contractor', function ($excel) use ($reportData, $fromDate, $toDate,$type) {
                    $excel->sheet('Sheet 1', function ($sheet) use ($reportData, $fromDate, $toDate,$type) {
                        $sheet->setOrientation('landscape');
                        $sheet->setFitToPage(1);
                        $sheet->loadView('reportexcel.apsofcontractorinterval')
                            ->with('type',$type)
        ...
        return View::make('report.apsofcontractorinterval')
            ->with('start',$start)
