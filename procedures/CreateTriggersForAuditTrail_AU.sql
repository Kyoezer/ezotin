        ...
        BLOCK: BEGIN
            ...
            cols_loop: LOOP
                ...
                IF end_of_cols = 1 THEN
                    ...
                    LEAVE cols_loop;
                END IF;
                IF col_name!='Id' AND col_name!='CreatedBy' AND col_name!='EditedBy' AND col_name!='CreatedOn' AND col_name!='EditedOn' THEN
                    ...
                    IF VForeignCount>0 THEN
                        ...
                        SET @updateStatement = CONCAT(@updateStatement,'\nif OLD.',col_name, ' != NEW.', col_name ,' then \nSET VForeignOldName=(SELECT Name FROM ',VForeignKeyTableName,' WHERE Id=OLD.',col_name,');\nSET VForeignNewName=(SELECT Name FROM ',VForeignKeyTableName,' WHERE Id=NEW.',col_name,'); \ninsert into sysaudittrail (TableName,ColumnName,OldValueId,OldValue,NewValueId,NewValue,RowId,Action,ActionDate,SysUserId,DbUser) values (''',vtable_name,''',''',col_name,''',OLD.',col_name,',VForeignOldName,NEW.',col_name,',VForeignNewName,NEW.Id,''U'',coalesce(NEW.EditedOn,NOW()),NEW.EditedBy,USER()); \nend if;' );
                    ELSE
                        SET @updateStatement = CONCAT(@updateStatement,'\nif OLD.',col_name, ' != NEW.', col_name ,' then \ninsert into sysaudittrail (TableName,ColumnName,OldValue,NewValue,RowId,Action,ActionDate,SysUserId,DbUser) values (''',vtable_name,''',''',col_name,''',OLD.',col_name,',NEW.',col_name,',NEW.Id,''U'',coalescce(NEW.EditedOn,NOW()),NEW.EditedBy,USER()); \nend if;' );
                    END IF;
                    SET VForeignCount=0;
                END IF;
            END LOOP cols_loop;
            close curc;
        END BLOCK;
        SET trigger_name = CONCAT('trig_',vtable_name,'_audit_au');
        SET @s = CONCAT(@s, '\nDROP TRIGGER IF EXISTS ', trigger_name ,';\nDELIMITER //\n\nCREATE TRIGGER ', trigger_name , ' AFTER UPDATE ON ', vtable_name, ' FOR EACH ROW \nBEGIN ' );
        SET @s = CONCAT(@s, '\nDECLARE VForeignCount INT DEFAULT 0;\nDECLARE VForeignKeyTableName VARCHAR(256) DEFAULT NULL;\nDECLARE VForeignOldName VARCHAR(500) DEFAULT NULL;\nDECLARE VForeignNewName VARCHAR(500) DEFAULT NULL;');
        SET @s = CONCAT(@s, @updateStatement,'\nEND//\nDELIMITER ;');
    END IF;
END LOOP;
CLOSE cur;
SET @filePath = CONCAT("'H:/Triggers_Ezotin" ,DATE_FORMAT(NOW(),'%d%b%Y%H%i%s'), ".sql'");
