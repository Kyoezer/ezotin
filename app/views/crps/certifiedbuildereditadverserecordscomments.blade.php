...
<div id="certifiedbuildercommentadverserecordedit" class="modal fade" role="dialog" aria-labelledby="certifiedbuildercommentadverserecordedit" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<input type="hidden" name="CrpCertifiedbuilderFinalId" value="{{$certifiedbuilder[0]->Id}}" />
...
<a href="{{URL::to('certifiedbuilder/editcommentsadverserecordslist')}}" class="btn grey-cascade">Back to List</a>
<h3 class="font-blue-madison">{{$certifiedbuilder[0]->NameOfFirm.' (Certified Builder No.'.$certifiedbuilder[0]->CDBNo.')'}}</h3>
<div class="row">
	<div class="col-md-12">
					...
					@forelse($commentsAdverseRecords as $commentsAdverseRecord)
						@if((int)$commentsAdverseRecord->Type==1)
							<div class="note note-info">
								...
								<input type="hidden" class="certifiedbuildercommentdate" value="{{convertDateToClientFormat($commentsAdverseRecord->Date)}}" />
								<input type="hidden" class="certifiedbuildercomment" value="{{$commentsAdverseRecord->Remarks}}" />
					...
					@forelse($commentsAdverseRecords as $commentsAdverseRecord)
						@if((int)$commentsAdverseRecord->Type==2)
							<div class="note note-info">
								...
								<input type="hidden" class="certifiedbuilderadverserecorddate" value="{{convertDateToClientFormat($commentsAdverseRecord->Date)}}" />
								<input type="hidden" class="certifiedbuilderadverserecord" value="{{$commentsAdverseRecord->Remarks}}" />
								<h5 class="bold">{{convertDateToClientFormat($commentsAdverseRecord->Date)}}</h5>
								<p>
									{{$commentsAdverseRecord->Remarks}}
								</p>
								<button type="button" class="btn green-haze btn-sm editcertifiedbuilderadverserecord">Edit</button>
								<button type="button" class="btn bg-red-sunglo btn-sm deletecertifiedbuilderfadverserecord">Delete</button>
							</div>
						@endif
					@empty
						<div class="note note-danger">
