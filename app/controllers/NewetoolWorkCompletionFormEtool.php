...
class NewetoolWorkCompletionFormEtool extends EtoolController{
	public function index($tenderId){
        ...
        return View::make('new_etool.workcompletionform')
	...
	public function listOfWorks(){
        ...
        $query = "select distinct `T1`.`Id`, case when T1.migratedworkid is null then 
        concat(T2.Code,'/',year(T1.UploadedDate),'/',T1.WorkId) else T1.migratedworkid 
        end as WorkId, `T2`.`Name` as `ProcuringAgency`, `T1`.`NameOfWork`,
         `T3`.`Name` as `Category`, `T4`.`Name` as `Status`,
         (select group_concat(concat(A.NameOfFirm,' (CDB No.:',A.CDBNo,')') SEPARATOR \", \") 
         from crpcontractorfinal A join etltenderbiddercontractordetail B on 
         A.Id = B.CrpContractorFinalId where B.EtlTenderBidderContractorId = T5.Id) 
         as Contractor, group_concat(T7.Id SEPARATOR \", \") as ContractorId,
          group_concat(T7.CDBNo SEPARATOR \", \") as ContractorNo from `etltender` as `T1`
           inner join (`cmnprocuringagency` as `T2` join sysuser as B on 
           B.CmnProcuringAgencyId = T2.Id) on `T1`.`CmnProcuringAgencyId` = `T2`.`Id` 
           inner join `cmncontractorworkcategory` as `T3` on `T3`.`Id` = `T1`.`CmnContractorCategoryId` 
           left join `cmnlistitem` as `T4` on `T4`.`Id` = `T1`.`CmnWorkExecutionStatusId` 
           left join `etltenderbiddercontractor` as `T5` on `T5`.`EtlTenderId` = `T1`.`Id` 
           and `T5`.`ActualStartDate` is not NULL left join `etltenderbiddercontractordetail` as `T6` 
           on `T6`.`EtlTenderBidderContractorId` = `T5`.`Id` join `crpcontractorfinal` as `T7` 
           on `T6`.`CrpContractorFinalId` = `T7`.`Id` where T1.TenderSource = 1 and 
           coalesce(T1.DeleteStatus,'N') <> 'Y' and T1.IsSPRRTender='Y'";
        $queryForDistinctYears = "select distinct case when T1.migratedworkid is null 
        then year(T1.UploadedDate) else SUBSTRING_INDEX(SUBSTRING_INDEX(T1.migratedworkid,'/',-2),'/',1) 
        end as Year from etltender T1 join (cmnprocuringagency A join sysuser B on
         A.Id = B.CmnProcuringAgencyId) on A.Id = T1.CmnProcuringAgencyId 
         left join `etltenderbiddercontractor` as `T5` on `T5`.`EtlTenderId` = `T1`.`Id` 
         and `T5`.`ActualStartDate` is not NULL left join `etltenderbiddercontractordetail` 
         as `T6` on `T6`.`EtlTenderBidderContractorId` = `T5`.`Id` left join `crpcontractorfinal` 
         as `T7` on `T6`.`CrpContractorFinalId` = `T7`.`Id` where T1.TenderSource = 1 and
          coalesce(T1.DeleteStatus,'N') <> 'Y'  and T1.IsSPRRTender='Y'";
        $appendToQuery = "";
        $appendToQuery2 = "";
        $userId = Auth::user()->Id;
        ...
        return View::make('new_etool.listofworks')
    ...
    public function postWorkCompletion(){
        ...
        $this->auditTrailEtoolCinet(NULL,NULL,$auditTrailActionMessage,$tenderDetails[0]->WorkId);
        return Redirect::to('newEtl/listofworksetool')->with('savedsuccessmessage','Tender has been updated successfully!');
    }
}