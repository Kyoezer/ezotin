var etool = function(){
    function checkPoints(){
        ...
        var flag = true;
        var hrTierIflag = false;
        var hrTierIVflag = false;
        var eqTierIflag = false;
        var countHR = 0;
        var countEq = 0;
        /* START HERE */
        ...
        hrTable.find('tbody tr:not(:last-child)').each(function(){
            ...
            var hrTierName = currentRow.find('option:selected','select[name$="[EtlTierId]"]').data('name');
           
            
            if(!isNaN(hrTier)){
                if(hrTierName == 'Tier I')
                {
                    hrTierIflag = true;
                }
                if(hrTierName == 'Tier IV')
                {
                    hrTierIVflag = true;
                }
        ...
        equipmentTable.find('tbody tr').each(function(){
            ...
            var equipmentTierName = currentRow.find('option:selected','select[name$="[EtlTierId]"]').data('name');
        ...
        if(!flag) {
            ...
            $('#modaldimessagetext').html("<strong>Please check your point allocation!<br/>For Human Resource: <br/>Tier I should have a maximum of 40 points<br/>Tier II a maximum of 30 points<br/>Tier III a maximum of 20 points<br/>Tier IV a maximum of 10 points<br/>It is not necessary to have all 4 Tiers<br/><br/>For Equipment:<br/> Tier 1 should have total of 50 points,<br/> Tier 2 a total of 30 points,<br/>Tier 3 a total of 20 points.<br/>It is not necessary to have all 3 Tiers</strong>");
            $('#modalmessagebox').modal('show');
            return false;
        }else if(!hrTierIflag && countHR>0){
            $('#modalmessageheader').html("<strong>Error !</strong>");
            $('#modaldimessagetext').html("<strong>HR Tier I is mandatory</strong>");
            $('#modalmessagebox').modal('show');
            return false;
        }else if(!hrTierIVflag && countHR>0){
            $('#modalmessageheader').html("<strong>Error !</strong>");
            $('#modaldimessagetext').html("<strong>HR Tier IV is mandatory</strong>");
            $('#modalmessagebox').modal('show');
            return false;
        }
        else if(!eqTierIflag && countEq>0){
            $('#modalmessageheader').html("<strong>Error !</strong>");
            $('#modaldimessagetext').html("<strong>Equipment Tier I is mandatory</strong>");
            $('#modalmessagebox').modal('show');
            return false;
        }
        else{
    ...
    function calculateTotal(table){
        ...
        table.find('.etool-total-SPRR').each(function(){
            var curElement = $(this);
            total += !isNaN(parseFloat(curElement.find(':selected').data('points')))?parseFloat(curElement.find(':selected').data('points')):0.00;
    ...
    function calculateTotal2(table){
        ...
        table.find('.etool-total-SPRR').each(function(){
            var curRow = $(this).parents('tr');
            total += !isNaN(parseFloat(curRow.find('.calculatedpoints').val())/0.20)?parseFloat(curRow.find('.calculatedpoints').val())/0.20:0.00;
            var maxEqPoints = parseFloat($('input[name="MaxEqPoints"]').val());
            var finalTotal = total/maxEqPoints * 20;
            if(finalTotal > 20){
                finalTotal = 20;
            }
            table.find('.total').val(finalTotal.toFixed(3));
    ...
    function initialize(){
        ...
        $('.nonResponsiveContractor').on('click',function(){
            $('#nonResponsiveContractor').modal("show");
        });
        $('.cancelTender').on('click',function(){
            $('#cancelTender').modal("show");
        });
        ...
        $(document).on('change','.EtlHrDesignationId',function(){
            ...
            var hrTierId = curRow.find('option:selected, .EtlHrQualificationIdSPRR').val();
            $('option:selected',curElement).addClass('selected');
            var curTierId = curRow.find('.EtlHrTierId').val();
            curRow.find('.EtlHrQualificationId').val('');
            curRow.find('.EtlHrQualificationIdSPRR').val('');
            curRow.find('.EtlHrPoints').val('');
            curRow.find('.EtlHrPointsReduced').val('');
            if(hrDesignationId != ''){
                ...
                curRow.find('.EtlHrQualificationIdSPRR option[value!=""]').each(function(){
                    if(($(this).data('designationid') == hrDesignationId) && ($(this).data('tierid') == hrTierId)){
                        $(this).removeClass('hide');
                        $(this).removeAttr('disabled');
                    }else{
        ...
        $(document).on('change','.EtlHrQualificationIdSPRR',function(){
            var table = $(this).parents('table');
            var curElement = $(this);
            var curRow = curElement.parents('tr');
            if($(this).val()!=''){
                var points = $('option:selected',curElement).data('points');
                var maxHrPoints = $('input[name="MaxHrPoints"]').val();
                var calculatedPoints = points * .20;
                curRow.find('.EtlHrPoints').val(calculatedPoints);
                curRow.find('.EtlHrPointsReduced').val(calculatedPoints);
            }else{
                curRow.find('.EtlHrPoints').val('');
        ...
        $(document).on('change','select[name$="[OwnedOrHiredSPRR]"]',function(){
            var table = $(this).parents('table');
            var curElement = $(this);
            var curRow = curElement.parents('tr');
            if($(this).val()!=''){
       ...
       // $(document).on('blur','table#ContractorAdd .cdbno',function(){
        ...
        // certified builder
        $(document).on('click','.cbreport-container a',function(e){
            e.preventDefault();
            var curElement = $(this);
            var href = $(this).attr('href');
            var param = $(this).parents('.form-group').find('input[type="text"]').val();
