	...
	<div class="portlet-body flip-scroll">
		...
		<td>
						@if((int)$consultantList->StatusReference == 12003)
							@if($consultantList->RegistrationExpiryDate<=date('Y-m-d G:i:s'))
									<p class="font-red bold warning">Expired </p>
							@else
								Valid/Approved
							@endif
						@else
							<p class="font-red bold warning">{{$consultantList->Status}}</p>
						@endif
					</td>
<!-- <td>
					...
					<td>
						<a href="{{URL::to($link.$consultantList->Id)}}" class="btn default btn-xs green-seagreen @if($linkText == 'Edit'){{" editaction"}}@endif" @if($linkText=="View/Print"){{"target='_blank'"}}@endif><i class="fa fa-edit"></i> {{$linkText}}</a>
					</td>
					<td>
						<a href="#changeexpirydatemodal" data-expirydate="{{convertDateToClientFormat($consultantList->RegistrationExpiryDate)}}" data-id="{{$consultantList->Id}}" data-toggle="modal" class="change-expirydate btn default btn-xs green"><i class="fa fa-edit"></i> Change</a>
					</td>
				</tr>
				@empty
				<tr>
