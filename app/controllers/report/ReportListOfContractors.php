...
class ReportListOfContractors extends ReportController{
	...
	$classificationId = Input::get('CmnContractorClassificationId');      
 	...
 	$limit = Input::has('Limit')?Input::get('Limit'):'All';
        ...
        $query = "select(select con.Email from crpcontractor con where con.CDBNo=T1.CDBNo limit 1) Email,(select a.Name from crpcontractorhumanresourcefinal a where a.CrpContractorFinalId IN (select b.Id from crpcontractorfinal b where b.CDBNo=T1.CDBNo) and a.IsPartnerOrOwner=1 limit 1) NAME,
        (select if(a.Sex='M','Male','Female') from crpcontractorhumanresourcefinal a where a.CrpContractorFinalId IN (select b.Id from crpcontractorfinal b where b.CDBNo=T1.CDBNo) and a.IsPartnerOrOwner=1 limit 1)gender,(select GROUP_CONCAT(distinct B.Name SEPARATOR ', ') from crpcontractortraining A join cmnlistitem B on B.Id = A.CmnTrainingModuleId join crpcontractortrainingdetail C on C.CrpContractorTrainingId = A.Id where C.CrpContractorFinalId = T1.Id and A.CmnTrainingTypeId = ?) as RefresherCourseModules,DATEDIFF(ExpiryDate,NOW()),CDBNo, NameOfFirm, Address,Status,Email, ExpiryDate, Dzongkhag, Country, TelephoneNo, MobileNo, Classification1, Classification2, Classification3, Classification4,OwnershipType
...
//die($classificationId.'-------------------------------');
    ...
    public function reportDetails(){
        ...
        else if($reportType=='Office_inspector' || $reportType=='Public ongoing site_inspector') 
        {
