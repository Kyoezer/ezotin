...
class EzotinReports extends ReportController{
	public function getApplicantsDueForPayment(){
        ...
        $contractorService = DB::select("SELECT T1.Id,T1.NameOfFirm,T1.CDBNo, T1.MobileNo,T1.ReferenceNo AS ApplicationNo,T1.ApplicationDate FROM `crpcontractor` T1  WHERE T1.`CmnApplicationRegistrationStatusId` LIKE '6195664d-c3c5-11e4-af9f-080027dcfac6'  ORDER BY T1.`ApplicationDate` DESC");
...
//        echo "<PRE>"; dd($designations);
    ...
    public function applicantsCancelledAfterNonPayment(){
        ...
        $parameters = array();
        $parametersForPrint = array();
        $query = "select case when T1.TypeCode = 1 then coalesce(T2.CDBNo,T3.CDBNo) else case when T1.TypeCode = 2 then coalesce(T4.CDBNo,T5.CDBNo) else case when T1.TypeCode = 3 then coalesce(T6.ARNo,T7.ARNo) else coalesce(T8.SPNo,T9.SPNo) end end end as CDBNo,case when T1.TypeCode = 1 then T2.CmnApplicationRegistrationStatusId else case when T1.TypeCode = 2 then T4.CmnApplicationRegistrationStatusId else case when T1.TypeCode = 3 then T6.CmnApplicationRegistrationStatusId else T8.CmnApplicationRegistrationStatusId end end end as StatusId,case when T1.TypeCode = 1 then T2.CDBNo else case when T1.TypeCode = 2 then T4.CDBNo else case when T1.TypeCode = 3 then T6.ARNo else T8.SPNo end end end as CDBNo,case when T1.TypeCode = 1 then T2.PaymentApprovedDate else case when T1.TypeCode = 2 then T4.PaymentApprovedDate else case when T1.TypeCode = 3 then T6.PaymentApprovedDate else T8.PaymentApprovedDate end end end as PaymentApprovedDate,case when T1.TypeCode = 1 then T2.ReferenceNo else case when T1.TypeCode = 2 then T4.ReferenceNo else case when T1.TypeCode = 3 then T6.ReferenceNo else T8.ReferenceNo end end end as ReferenceNo,
