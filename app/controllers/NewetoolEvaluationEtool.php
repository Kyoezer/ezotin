<?php
class NewetoolEvaluationEtool extends EtoolController{
	public function index(){
       ...
       IF (
        a.`Name` IS NULL,
        IF(
          b.`EtlTenderId` IS NULL,
          IF(T1.`TenderStatus`IS NULL OR T1.`TenderStatus` = '','Under Process',T1.`TenderStatus`),
          'Evaluated'
        ),
        a.`Name`
      ) tenderStatus,

        T1.`Method`,T1.`TenderStatus`,T1.`EGPTenderId`,  T1.TenderOpeningDateAndTime, T2.Code as Category, T3.Code as Classification, T1.NameOfWork, T1.ContractPeriod,
         case when T1.migratedworkid is null then concat(a.Code,'/',year(T1.UploadedDate),'/',T1.WorkId) else T1.migratedworkid end as EtlTenderWorkId from etltender T1
         join cmncontractorworkcategory T2 on T1.CmnContractorCategoryId = T2.Id join cmncontractorclassification T3 on T1.CmnContractorClassificationId = T3.Id join 
         (cmnprocuringagency Ab join sysuser Bb on Ab.Id = Bb.CmnProcuringAgencyId) on Ab.Id = T1.CmnProcuringAgencyId 
        LEFT JOIN `cmnlistitem` a ON T1.CmnWorkExecutionStatusId=a.id
        LEFT JOIN `etlevaluationscore` b ON T1.Id=b.`EtlTenderId`
        where T1.IsSPRRTender='Y' and T1.CmnProcuringAgencyId = ? and T1.TenderSource = 1 and coalesce(T1.DeleteStatus,'N') <> 'Y'";
        ...
        if((bool)$workId || (bool)$contractorCategoryId || (bool)$contractorClassificationId) {
            if ((bool)$workId) {
                $query1 .= " and case when T1.migratedworkid is null then concat(a.Code,'/',year(T1.UploadedDate),'/',T1.WorkId) else T1.migratedworkid end LIKE '%$workId%'";
                $queryForDistinctYears .= " and concat(a.Code,'/',DATE_FORMAT(T1.UploadedDate,'%Y'),'/',T1.WorkId) LIKE '%$workId%'";
            }
        ...
        IF (
         ...
         T1.`Method`,T1.`TenderStatus`,T1.`EGPTenderId`, T1.TenderOpeningDateAndTime, T2.Code as Category, T3.Code as Classification, T1.NameOfWork,
          T1.ContractPeriod, case when T1.migratedworkid is null then concat(a.Code,'/',year(T1.UploadedDate),'/',T1.WorkId) else T1.migratedworkid end as EtlTenderWorkId from etltender T1 
          join cmncontractorworkcategory T2 on T1.CmnContractorCategoryId = T2.Id join cmncontractorclassification T3 on T1.CmnContractorClassificationId = T3.Id 
          join (cmnprocuringagency Ae join sysuser Be on Ae.Id = Be.CmnProcuringAgencyId) on Ae.Id = T1.CmnProcuringAgencyId 
         LEFT JOIN `cmnlistitem` a ON T1.CmnWorkExecutionStatusId=a.id
        LEFT JOIN `etlevaluationscore` b ON T1.Id=b.`EtlTenderId`
        where T1.IsSPRRTender='Y' and b.Id = ? and T1.TenderSource = 1 and T3.ReferenceNo in (3,4) and
         coalesce(T1.DeleteStatus,'N') <> 'Y'  and T1.CmnContractorClassificationId not in ('ef832830-c3ea-11e4-af9f-080027dcfac6')  ";
...
//
		...
		return View::make('new_etool.evaluationlist')
    ...
    public function addNonResponsive(){
        ...
        $save = true;
        DB::commit();
        return Redirect::to('newEtl/workevaluationdetails/'.$inputs['tenderId'])->with('savedsuccessmessage',$save?'Record has been saved':"Record has been updated");
    ...
    public function pushToNonResponsive(){
        ...
        $save = true;
        return Redirect::to('newEtl/workevaluationdetails/'.$inputs['tenderId'])->with('savedsuccessmessage',$save?'Record has been updated':"Record has been updated");
	...
	public function evaluationCommittee($tenderId){
        ...
        return View::make('new_etool.evaluationcommitee')
    ...
    public function awardingCommittee($tenderId){
        ...
        $this->auditTrailEtoolCinet(NULL,NULL,$auditTrailActionMessage,$tenders[0]->WorkId);
        return View::make('new_etool.awardingcommittee')
            ->with('tenders',$tenders)
            ->with('committee',$committee)
            ->with('tenderId',$tenderId);
    }
    public function saveCommittee(){
        ...
        return Redirect::to('newEtl/'.$redirectRoute.'/'.$inputArray['EtlTenderId'])->with('savedsuccessmessage','Record has been saved!');
    }
	public function details($tenderId = NULL){
        ...
        $lowestBid = DB::table('etltenderbiddercontractor as T1')
            ...
            ->whereRaw("(T2.Score1 + T2.Score2 + T2.Score3 + T2.Score4 + T2.Score5 + T2.Score6 + T2.Score_Works) >= $qualifyingScore")
          ...
          GROUP BY `T1`.`Id`');
            ...
            // $contractors = DB::table('etltenderbiddercontractor as T1')
        ...
        //     ->where('T1.Id','=',$tenderId)
        //     ->where('T4.Id','=',Auth::user()->Id)
        //     ->get(array('T1.Id','T1.Method','T1.CmnWorkExecutionStatusId','T1.ProjectEstimateCost','T1.TentativeStartDate','T1.TentativeEndDate',DB::raw("case when T1.migratedworkid is null then concat(T5.Code,'/',year(T1.UploadedDate),'/',T1.WorkId) else T1.migratedworkid end as WorkId"),"T5.Name as ProcuringAgency","T1.DescriptionOfWork","T1.NameOfWork",DB::raw("concat(T2.Name,' (',T2.Code,')') as Classification"),"T3.Code as Category","T7.NameEn as Dzongkhag"));
        
        $auditTrailActionMessage="Clicked details button in evaluation to view the details of work Id ".$tenders[0]->WorkId;
        $this->auditTrailEtoolCinet(NULL,NULL,$auditTrailActionMessage,$tenders[0]->WorkId);
        return View::make('new_etool.workevaluationdetails')
	...
	public function addSmallContractors($tenderId,$contractorId=NULL){
        ...
        $tenders = DB::table('etltender as T1')
            ...
            ->where('T1.Id','=',$tenderId)
            ->get(array('T1.Id',DB::raw("case when T1.migratedworkid is null then concat(T3.Code,'/',year(T1.UploadedDate),'/',T1.WorkId) else T1.migratedworkid end as WorkId"),'T4.NameEn as Dzongkhag','T1.NameOfWork','T1.DescriptionOfWork'));
...
//            ->orderBy('T4.Name')
		...
		return View::make('new_etool.evaluationaddsmallcontractors')
                    ->with('hasEq',$hasEq)
                    ->with('hasHr',$hasHr)
...
//        return $contractorList;
        ...
        return View::make('new_etool.evaluationaddcontractors')
                    ->with('hasEq',$hasEq)
                    ->with('hasHr',$hasHr)
                    ->with('contractorCDBNos',$contractorCDBNos)
                    // ->with('vtiEmployment',$vtiEmployment)
        ...
        foreach($contractorsList as $contractor){
	     ...
	     $quotedAmount = DB::table('etltenderbiddercontractor')->where('Id',$contractor->Id)->where('EtlTenderId',$tenderId)->pluck('FinancialBidQuoted');
            ...
            foreach($contractors as $singleContractor) {
                ...
                if ($loopCount == 1){
                    $equipmentResultArray[$contractor->Id] = $this->calculatePointsOnEquipmentSPRR($tenderId, $singleContractor->CrpContractorFinalId,$maxEqPoints);
                    $hrResultArray[$contractor->Id] = $this->calculatePointsOnHRSPRR($tenderId, $singleContractor->CrpContractorFinalId,$maxHrPoints);
                }
...
//            }
            try{
                ...
                if(isset($id) && !empty($id)){
                    ...
                    $avgAPS = number_format($performanceResultArray[$contractor->Id],2)* 10;
                    $workScore = 0;
                    if($avgAPS>=75)
                    {
                        $workScore = 10;
        ...
        $lowestBid = DB::table('etltenderbiddercontractor as T1')
            ->join('etlevaluationscore as T2','T2.EtlTenderBidderContractorId','=','T1.Id')
            ->where('T1.EtlTenderId','=',$tenderId)
            ->where(DB::raw("(Score1+Score2+Score3+Score4+Score5+Score6+Score_Works)"),'>=',DB::raw("$qualifyingScore"))
            ->min('T1.FinancialBidQuoted');
        ...
        return Redirect::to('newEtl/workevaluationdetails/'.$tenderId)->with('savedsuccessmessage','Result has been processed');
	}
    public function resetResult($tenderId){
        ...
        return Redirect::to('newEtl/workevaluationdetails/'.$tenderId)->with('savedsuccessmessage','Result has been reset');;
    }
	public function pointDetails($tenderId,$contractorId){
        ...
        $points = DB::table('etlevaluationscore')
                        ...
                        ->get(array('Score1','Score2','Score3','Score4','Score5','Score6','Score7','Score11','Score_Works'));
		...
		return View::make('new_etool.evaluationpointdetails')
	...
	public function detailsSmall($tenderId,$contractorId=NULL){
        ...
        $tenders = DB::table('etltender as T1')
            ...
            ->where('T1.Id','=',$tenderId)
            ->get(array(DB::raw('distinct(T1.Id)'),'T1.CmnWorkExecutionStatusId',DB::raw("case when T1.migratedworkid is null then concat(T3.Code,'/',year(T1.UploadedDate),'/',T1.WorkId) else T1.migratedworkid end as WorkId"),'T1.TentativeStartDate','T1.TentativeEndDate','T1.NameOfWork','T1.DescriptionOfWork','T1.ProjectEstimateCost'));
        $allContractors = ContractorFinalModel::contractorHardListAll()->get(array('Id','NameOfFirm','CDBNo'));
		
        
        
        return View::make('new_etool.workevaluationdetailssmall')
            ->with('contractors',$contractors)
    ...
    public function postSaveAddContractor(){
        ...
        if(isset($parentTableInputs['FinancialBidQuoted'])){
            ...
            foreach($contractorListInputs as $key=>$value){
                ...
                if($contractorStatus[0]->CmnApplicationRegistrationStatusId != CONST_CMN_APPLICATIONREGISTRATIONSTATUS_APPROVED){
                    ...
                    if($ContractorType=='LARGE')
                    {
                        return Redirect::to('newEtl/workevaluationaddcontractors/'.$parentTableInputs['EtlTenderId'])->with('customerrormessage',"This contractor ($contractorName) is ". $contractorStatus[0]->Status);
                    }
                    else if($ContractorType=='SMALL')
                    {
                        return Redirect::to('newEtl/smallWorkevaluationaddcontractors/'.$parentTableInputs['EtlTenderId'])->with('customerrormessage',"This contractor ($contractorName) is ". $contractorStatus[0]->Status);
                    }
                ...
                if((int)$contractorExistsCheck > 0){
                    ...
                    if($ContractorType=='LARGE')
                    {
                        return Redirect::to('newEtl/workevaluationaddcontractors/'.$parentTableInputs['EtlTenderId'])->with('customerrormessage',"This contractor ($contractorName) has already bidded for this tender");
                    }
                    else if ($ContractorType=='SMALL')
                    {
                        return Redirect::to('newEtl/smallWorkevaluationaddcontractors/'.$parentTableInputs['EtlTenderId'])->with('customerrormessage',"This contractor ($contractorName) has already bidded for this tender");
                    }
                }
                try{
                    $childTableInputs['EtlTenderBidderContractorId'] = $parentTableInputs['Id'];
            ...
            if(Input::has('Source')){
                ...
                return Redirect::to('newEtl/smallWorkevaluationaddcontractors/'.$parentTableInputs['EtlTenderId'].'/'.$parentTableInputs['Id'])->with('savedsuccessmessage',$save?"Record has been saved":"Record has been updated");
            }
            DB::commit();
            $contractorFirmName=DB::table('etltenderbiddercontractordetail as T1')
                                ->join('crpcontractorfinal as T2','T1.CrpContractorFinalId','=','T2.Id')
            ...
            if($ContractorType=='SMALL')
                ...
                return Redirect::to('newEtl/workevaluationaddcontractors/'.$parentTableInputs['EtlTenderId'].'/'.$parentTableInputs['Id'])->with('savedsuccessmessage',$save?"Record has been saved":"Record has been updated");
        }else{
            $save = false;
            $childTableInputs = array();
        ...
        if($ContractorType=='SMALL')
        {    
            return Redirect::to('newEtl/smallWorkevaluationaddcontractors/'.$parentTableInputs['EtlTenderId'].'/'.$parentTableInputs['Id'].'?currentTab='.$currentTabParameter.$currentTab)->with('savedsuccessmessage',$save?'Record has been saved':"Record has been updated");
        }
        else if($ContractorType=='LARGE')
        {
            return Redirect::to('newEtl/workevaluationaddcontractors/'.$parentTableInputs['EtlTenderId'].'/'.$parentTableInputs['Id'].'?currentTab='.$currentTabParameter.$currentTab)->with('savedsuccessmessage',$save?'Record has been saved':"Record has been updated");
        }
    ...
    public function etlPostSeekClarification(){
       ...
       // die($inputs['Tender_Id'].'/'.$inputs['CDB_No'].'/'.$inputs['contractorId']);
        ...
        return View::make('new_etool.seekClarificationList')
    ...
    public function postAwardContractor(){
        ...
        if($change){
            return Redirect::to('newEtl/etoolchangetenderawarded?WorkId='.$workIdForAuditTrail[0]->WorkId)->with('savedsuccessmessage','Work awarded to has been changed');
        }
        $redirect = 'newEtl/workevaluationdetails';
        if(Input::has('WorkType')){
            $redirect = 'newEtl/workevaluationdetailssmallcontractors';
        }
    ...
    public function processResultLargetoSmall($tenderId){
        ...
        $this->auditTrailEtoolCinet(NULL,NULL,$auditTrailActionMessage,$workIdForAuditTrail[0]->WorkId);
        DB::table('etlevaluationtrack')->insert(array('Id'=>$this->UUID(),'WorkId'=>$workIdForAuditTrail[0]->WorkId,'Operation'=>'Process','User'=>Auth::user()->FullName,'OperationTime'=>date('Y-m-d G:i:s')));
        return Redirect::to('newEtl/workevaluationdetailssmallcontractors/'.$tenderId)->with('savedsuccessmessage','Result has been processed');
    }
    ...
    public function resetResultSmall($tenderId){
        ...
        $this->auditTrailEtoolCinet(NULL,NULL,$auditTrailActionMessage,$workIdForAuditTrail[0]->WorkId);
        DB::table('etlevaluationtrack')->insert(array('Id'=>$this->UUID(),'WorkId'=>$workIdForAuditTrail[0]->WorkId,'Operation'=>'Reset Result','User'=>Auth::user()->FullName,'OperationTime'=>date('Y-m-d G:i:s')));
        return Redirect::to('newEtl/workevaluationdetailssmallcontractors/'.$tenderId)->with('savedsuccessmessage','Result has been reset');
    }
    public function getResultReport($tenderId){
        ...
        $contractorScores = DB::table('etlevaluationscore as T1')
                            ...
                            ->orderBy(DB::raw('coalesce(T1.Score10,0)'),'DESC')
                            ->groupBy('T2.Id')
                            ->get(array('T1.Id as ScoreId','T2.ActualStartDate',DB::raw('group_concat(concat(T5.NameOfFirm, "(",T5.CDBNo,")") SEPARATOR ", ") as CDBNo'), 'T2.FinancialBidQuoted','T1.Score1','T1.Score2','T1.Score3','T1.Score4','T1.Score5','T1.Score6','T1.Score7','T1.Score8','T1.Score9','T1.Score10','T1.Score11','T1.Score_Works','T1.IsBhutaneseEmp'));
	$lowestBid = DB::table('etltenderbiddercontractor as T1')
            ->join('etlevaluationscore as T2','T2.EtlTenderBidderContractorId','=','T1.Id')
            ->where('T1.EtlTenderId','=',$tenderId)
            ->whereRaw("(T2.Score1 + T2.Score2 + T2.Score3 + T2.Score4 + T2.Score5 + T2.Score6 + T2.Score_Works) >= $qualifyingScore")
            ->min('T1.FinancialBidQuoted');
        return View::make('new_etool.resultreport')
                    ->with('qualifyingScore',$qualifyingScore)
                    ->with('tenderDetails',$tenderDetails)
    ...
    public function getSmallWorksReport($tenderId){
        ...
        return View::make('new_etool.smallresultreport')
                    ->with('lowestBid',$lowestBid)
                    ->with('tenderDetails',$tenderDetails)
                    ->with('bidContractors',$bidContractors)
                    ->with('evaluationCommittee',$evaluationCommittee)
