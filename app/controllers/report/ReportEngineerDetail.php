...
class ReportEngineerDetail extends ReportController
{
 ...
 $statuses = CmnListItemModel::registrationStatus()->get(array('Id','Name'));
   ...
   if((bool)$status){
        ...
        if(Input::has('export')){
            ...
            if($export == 'excel'){
                Excel::create('Engineer Application Details', function($excel) use ($engineerLists,$parametersForPrint) {
                    $excel->sheet('Sheet 1', function($sheet) use ($engineerLists,$parametersForPrint) {
                        $sheet->setOrientation('landscape');
                        $sheet->setFitToPage(1);
                        $sheet->setPaperSize(1);
                        $sheet->loadView('reportexcel.engineerdetail')
        ...
        return View::make('report.engineerdetail')
            ->with('start',$start)
