...
class ReportListOfSurveyors extends ReportController{
    public function getIndex(){
        ...
        $query = "select ARNo,Sector,Status, CIDNo,SurveyName, ApprovedDate,ExpiryDate, Dzongkhag, Country, Gewog,Village,Email, MobileNo from viewlistofsurveys Where 1";
        ...
        if(Input::has('export')){
            $surveyList = DB::select($query." order by ARNo",$parameters);
            $export = Input::get('export');
            if($export == 'excel'){
                Excel::create('List of Surveyors', function($excel) use ($surveyList, $country,$dzongkhag,$arNo,$sector,$limit,$fromDate,$toDate,$status) {

                    $excel->sheet('Sheet 1', function($sheet) use ($surveyList, $country,$dzongkhag,$arNo,$sector,$limit,$fromDate,$toDate,$status) {
                        $sheet->setOrientation('landscape');
                        $sheet->setFitToPage(1);
                        $sheet->loadView('reportexcel.listofsurveyors')
            ...
            $surveyList  = DB::select($query." order by ARNo".$limitOffsetAppend,$parameters);
        }
        return View::make('report.listofsurveyors')
                        ->with('start',$start)
