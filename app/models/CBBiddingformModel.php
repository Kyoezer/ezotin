...
class CBBiddingformModel extends BaseModel{
    ...
    protected $messages=array(
     ...
     public function scopebiddingFormContractorCdbAll($query){
        return $query->leftjoin('cmnprocuringagency as T1','cbbiddingform.CmnProcuringAgencyId','=','T1.Id')
                    ->leftjoin('cmncontractorworkcategory as T2','cbbiddingform.CmnContractorProjectCategoryId','=','T2.Id')
                    ->leftjoin('cmncontractorclassification as T3','cbbiddingform.CmnContractorClassificationId','=','T3.Id')
                    ->leftjoin('cmndzongkhag as T4','cbbiddingform.CmnDzongkhagId','=','T4.Id')
                    ->where('cbbiddingform.Type',0)
                    ->orderBy('T1.Name');
    }

   

    public function scopebiddingFormCertifiedBuilderCdbAll($query){
        return $query->leftjoin('cmnprocuringagency as T1','cbbiddingform.CmnProcuringAgencyId','=','T1.Id')
                    ->leftjoin('cmncertifiedbuildercategory as T2','cbbiddingform.CmnCertifiedBuilderCategoryId','=','T2.Id')
                    ->leftjoin('cmndzongkhag as T4','cbbiddingform.CmnDzongkhagId','=','T4.Id')
                    ->where('cbbiddingform.Type',2)
                    ->orderBy('T1.Name');
    }
