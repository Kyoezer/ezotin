<?php
class EtoolController extends BaseController{
    public function __construct(){
        $route = Request::segment(1).'/'.Request::segment(2);
        if($route != "etl/mydashboard"){
            $email = Auth::user()->Email;
            if(!(bool)$email){
                return Redirect::to('etl/mydashboard')->with('customerrormessage',"<strong>WARNING!</strong> To continue using Etool, please update your email address.")->send();
            }
        }
        $userId = Auth::user()->Id;
...
//            }
    ...
    public function calculatePointsOnEquipmentSPRR($tenderId, $contractorId,$maxPoints){
        $points = DB::table('etlcontractorequipment as T1')
            ->join('etltenderbiddercontractor as T2','T1.EtlTenderBidderContractorId','=','T2.Id')
            ->join('etltenderbiddercontractordetail as T3','T3.EtlTenderBidderContractorId','=','T2.Id')
            ->where('T2.EtlTenderId','=',$tenderId)
        ...
        $finalPoints = (($points/0.20)/$maxPoints * 20);
        if($finalPoints > 20){
            $finalPoints = 20;
        }
        return $finalPoints;
    ...
    public function calculatePointsOnHRSPRR($tenderId,$contractorId,$maxPoints){
        $points = DB::table('etlcontractorhumanresource as T1')
                            ->join('etltenderbiddercontractor as T2','T1.EtlTenderBidderContractorId','=','T2.Id')
                            ->join('etltenderbiddercontractordetail as T3','T3.EtlTenderBidderContractorId','=','T2.Id')
                            ->where('T2.EtlTenderId','=',$tenderId)
