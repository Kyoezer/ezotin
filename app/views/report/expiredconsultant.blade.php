...
<div class="portlet light bordered">
	<div class="portlet-title">
		<div class="caption">
            <?php $parameters = Input::all(); $parameters['export']='excel';  if($type == 1): $route = 'consultantrpt.expiredconsultant'; else: if($type==3): $route="consultantrpt.listofconsultantsnearingexpiry"; else: $route = 'consultantrpt.listofnonbhutaneseconsultants'; endif; endif;?>
                <i class="fa fa-cogs"></i>List of @if($type == 2){{"Non Bhutanese "}}<?php endif; ?>Expired Consultants @if($type ==3){{"whose registration is expiring"}}@endif &nbsp;&nbsp;@if(!Input::has('export')) <a href="{{route($route,$parameters)}}" class="btn btn-sm green"><i class="fa fa-file-excel-o"></i> Export to Excel </a>   <?php $parameters['export'] = 'print'; ?><a href="{{route($route,$parameters)}}" class="btn btn-sm purple" target="_blank"><i class="fa fa-print"></i> Print</a>@endif
		</div>
	</div>
	<div class="portlet-body flip-scroll">
        @if(Input::get('export') != 'print')
            @if($type == 1)
                <?php $url = 'consultantrpt/expiredconsultant'; ?>
            @else
                @if($type == 3)
