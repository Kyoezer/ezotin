...
{{ Form::open(array('url' => 'specializedfirm/mspecializedfirmhumanresource','role'=>'form','files'=>true))}}
				...
				<div class="row">
					<div class="col-md-12 table-responsive">
						...
						<table class="table table-bordered table-striped table-condensed">
							{{Form::hidden('Model','SpecializedfirmHumanResourceAttachmentFinalModel',array('class'=>'delete-model'))}}
							<thead>
								<tr>
						...
						<table class="table table-bordered table-striped table-condensed">
							{{Form::hidden('Model','SpecializedfirmHumanResourceAttachmentModel',array('class'=>'delete-model'))}}
							<tbody>
...
@if((int)$serviceBySpecializedtrade==1)
	<input type="hidden" name="serviceBySpecializedtrade" value="{{$serviceBySpecializedtrade}}">
@endif
{{Form::close()}}
<div class="portlet light bordered">
		...
		<table class="table table-bordered table-striped table-condensed flip-content">
			...
			<thead>
				<tr>
					...
					{{--@if((int)$serviceBySpecializedtrade==0)--}}
					<th>
						Delete Request
					</th>
					<th scope="col">
			...
			<tbody>
				@forelse($specializedtradeHumanResourcesFinal as $specializedtradeHumanResourceFinal)
				<tr>
					<td>
						<input type="hidden" class="rowreference" value="{{$specializedtradeHumanResourceFinal->Id}}"/>
						{{$specializedtradeHumanResourceFinal->Name}}
					</td>
					<td>{{$specializedtradeHumanResourceFinal->CIDNo}}</td>
					<td>{{$specializedtradeHumanResourceFinal->Sex}}</td>
					<td>{{$specializedtradeHumanResourceFinal->Country}}</td>
					<td>{{$specializedtradeHumanResourceFinal->Qualification}}</td>
				
					<td>{{$specializedtradeHumanResourceFinal->Designation}}</td>
					<td>{{$specializedtradeHumanResourceFinal->JoiningDate}}</td>
					<td>{{$specializedtradeHumanResourceFinal->Trade}}</td>
					<td>
						@foreach($humanResourcesAttachmentsFinal as $humanResourcesAttachmentFinal)
						@if($specializedtradeHumanResourceFinal->Id==$humanResourcesAttachmentFinal->CrpSpecializedtradeHumanResourceFinalId)
					...
					<td>
						<center><input type="checkbox" class="senddeleterequest" data-type="1" value="{{$specializedtradeHumanResourceFinal->Id}}"></center>
					</td>
					{{--@if((int)$serviceBySpecializedtrade==0)--}}
					<td>
						@if((bool)$isEdit!=null)
							@if(!empty($isEditByCDBUser))
							<a href="{{URL::to($humanResourceEditRoute.'/'.$isEdit.'?humanresourceid='.$specializedtradeHumanResourceFinal->Id.'&redirectUrl='.$redirectUrl.'&usercdb=edit')}}" class="editaction">Edit</a>
							@else
								<a href="{{URL::to($humanResourceEditRoute.'/'.$isEdit.'?humanresourceid='.$specializedtradeHumanResourceFinal->Id.'&redirectUrl='.$redirectUrl)}}" class="editaction">Edit</a>
							@endif
	...
	<div class="table-responsive">
		...
		<table class="table table-bordered table-striped table-condensed flip-content">
			...
			{{Form::hidden('Model',$changeModel?'SpecializedfirmHumanResourceFinalModel':'SpecializedfirmHumanResourceModel',array('class'=>'delete-model'))}}
			<thead>
				<tr>
					<th scope="col">
						 Name
...
{{--					@if((int)$serviceBySpecializedtrade==0)--}}
			...
			<tbody>
				...
				<tr>
					<td>
						<input type="hidden" class="rowreference" value="{{$specializedtradeHumanResource->Id}}">
						{{$specializedtradeHumanResource->Name}}
					</td>
					...
					<td>
						<input type="hidden" class="recordreference" value="{{$specializedtradeHumanResource->Id}}">
						<a href="{{URL::to($humanResourceEditRoute.'/'.$isEdit.'?humanresourceid='.$specializedtradeHumanResource->Id.'&redirectUrl='.$redirectUrl.'&usercdb=edit&initial=true')}}" class="editaction">Edit</a> |
	...
	<div class="form-actions">
		<div class="btn-set">
			...
			<a href="{{URL::to($nextUrl.$specializedtradeId)}}" class="btn blue button-next">
				Done <i class="m-icon-swapright m-icon-white"></i>
			</a>
			@else
			<a href="{{URL::to($redirectUrl.'/'.$specializedtradeId)}}" class="btn blue button-next">
				Done <i class="m-icon-swapright m-icon-white"></i>
			</a>
			@endif
		</div>
