...
class ReportListOfContractorExpiry  extends ReportController{
    public function getIndex(){
        ...
        $query = " SELECT CDBNo, NameOfFirm, Address, Status, ExpiryDate, ApplicationDate,Country, Dzongkhag, TelephoneNo, MobileNo, Classification1, Classification2, Classification3, Classification4, ownerName, gender FROM listofContractorExpiry where 1";
        ...
        if(Input::has('export')){
            ...
            if($export == 'excel'){
                Excel::create('List of Contractors nearing Expiry', function($excel) use ($contractorList, $country,$dzongkhag,$cdbNo,$limit,$fromDate,$toDate,$start) {

                    $excel->sheet('Sheet 1', function($sheet) use ($contractorList, $country,$dzongkhag,$cdbNo,$limit,$fromDate,$toDate,$start) {
                        $sheet->setOrientation('landscape');
                        $sheet->setFitToPage(1);
                        $sheet->loadView('reportexcel.listofexpirycontractor')
        ...
        return View::make('report.listofexpirycontractor')
                        ->with('start',$start)
