...
{{ Form::open(array('url' =>Request::URL(),'role'=>'form','method'=>'get')) }}
	<h4 class="text-primary"><strong><i class="fa fa-list"></i> List of Certified Builders</strong></h4>
	<div class="alert alert-info">
		<p>Search certified builder by selecting relevant parameters. If you select more than one parameter, the result will be combination of selected parameters.</p>
	</div>
...
{{Form::close()}}<br />
		...
		<table class="table table-bordered table-striped table-condensed flip-content" id="contractorhumanresource">
			...
			<tbody>
            @forelse($certifiedbuilderList as $certifiedbuilder)
				<tr>
                    <td>{{$start++}}</td>
                    <td>{{$certifiedbuilder->CDBNo}}</td>
                    <td>{{$certifiedbuilder->NameOfFirm}}</td>
                    <td>{{$certifiedbuilder->Address}}</td>
                    <td>{{$certifiedbuilder->Country}}</td>
                    <td>{{$certifiedbuilder->Dzongkhag}}</td>                
                    <td>{{$certifiedbuilder->ExpiryDate}}</td>
                    <td>{{$certifiedbuilder->Status}}</td>
				</tr>
            @empty
                <tr>
